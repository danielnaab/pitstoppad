...
def js_tag(filename, media_dir=settings.MEDIA_URL):
    tag = "<script src=\"%s%s\" type=\"text/javascript\"></script>" % (media_dir, _cleanfilename(filename) + ".js?" + _hashit(filename))
    
    return tag 

def css_tag(filename, media_dir=settings.MEDIA_URL):
    tag = "<link rel=\"stylesheet\" href=\"%s%s\" />" % (media_dir, _cleanfilename(filename) + ".css?" + _hashit(filename))
